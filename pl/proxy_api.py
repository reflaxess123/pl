...
class ProxyAPIClient:
    ...
    def __init__(self, api_key: Optional[str] = None, system_prompt: Optional[str] = None):
        ...
        self.system_prompt = system_prompt or os.getenv('SYSTEM_PROMPT') or (
            ...
            "ВАЖНО: Если в запросе есть 'Контекст беседы:', анализируй и используй эту информацию для ответа. "
            "Это реальные сообщения из чата. Отвечай на основе предоставленного контекста. "
            "Твоя цель - выполнять команды точно и качественно, писать естественно и по делу. "
            "Отвечай кратко и по существу, избегай фраз 'я не могу' или 'как ИИ'."
        )
